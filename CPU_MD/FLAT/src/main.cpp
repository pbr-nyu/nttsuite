...
int main(int argc, char **argv){
	...
	DATA_TYPE naiveResult[VECTOR_SIZE], nttResult[VECTOR_ADDR_BIT + 1][VECTOR_SIZE];
	randVec(vec, vec2, 1000);
	gettwiddle(twiddle, twiddle_h, p, r);

	
	ntt_flat(vec, p, r, twiddle, twiddle_h, nttResult);
	bit_reverse(nttResult[VECTOR_ADDR_BIT], VECTOR_SIZE, vec);
