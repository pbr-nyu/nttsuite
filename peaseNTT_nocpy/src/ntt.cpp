...
void peaseNTT(DATA_TYPE xt[VECTOR_SIZE], DATA_TYPE p, DATA_TYPE r, DATA_TYPE twiddle[VECTOR_SIZE], DATA_TYPE twiddle_h[VECTOR_SIZE]){
	...
	STAGE_LOOP: for (PARAMS_TYPE c = VECTOR_ADDR_BIT; c >= 1; c -= 2){

        	PARAMS_TYPE base = -1 << (c - 1);

        	INNER_LOOP1: for (PARAMS_TYPE r = 0; r < mid; r += 8){
			butterFly(xt, yt, r + 0, p, base, twiddle, twiddle_h);
			...
			butterFly(xt, yt, r + 4, p, base, twiddle, twiddle_h);
			butterFly(xt, yt, r + 5, p, base, twiddle, twiddle_h);
			butterFly(xt, yt, r + 6, p, base, twiddle, twiddle_h);
			butterFly(xt, yt, r + 7, p, base, twiddle, twiddle_h);
        	}

        	base = -1 << (c - 2);
        	INNER_LOOP2: for (PARAMS_TYPE r = 0; r < mid; r += 8){
			butterFly(yt, xt, r + 0, p, base, twiddle, twiddle_h);
			butterFly(yt, xt, r + 1, p, base, twiddle, twiddle_h);
			butterFly(yt, xt, r + 2, p, base, twiddle, twiddle_h);
			butterFly(yt, xt, r + 3, p, base, twiddle, twiddle_h);
			butterFly(yt, xt, r + 4, p, base, twiddle, twiddle_h);
			butterFly(yt, xt, r + 5, p, base, twiddle, twiddle_h);
			butterFly(yt, xt, r + 6, p, base, twiddle, twiddle_h);
			butterFly(yt, xt, r + 7, p, base, twiddle, twiddle_h);
        	}
